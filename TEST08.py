...
def region_of_interest(r_image):
    ...
    poly = np.array([
                    [(80, h-80), (480, h-80), (330, 200), (280, 200)] #a02.jpg
                    ])
    mask = np.zeros_like(r_image)
    cv2.fillPoly(mask, poly, 255)
    #cv2.imshow('fillPoly',mask)
    ...
    while(True):
        ...
        if ret:
            try:
                ...
                lines = cv2.HoughLinesP(cropped_image, 1, np.pi / 180, 20, np.array([]), minLineLength=80, maxLineGap=80)
                #lines = cv2.HoughLinesP(cropped_image, 2, np.pi / 180, 60, np.array([]), minLineLength=180, maxLineGap=60)
                get_lines(lines) # 分別得到左右車道線的圖片

                good_leftlines = choose_lines(lefts, 0) # 處理後的點
                good_rightlines = choose_lines(rights, 0)
                ...
                lefttop = clac_edgepoints(leftpoints, 240, image.shape[0]) # 要畫左右車道線的端點
                righttop = clac_edgepoints(rightpoints, 240, image.shape[0])
